...
Como se puede observar, en la mayoría de los casos de optimización para una única corrida los puntos óptimos conergen a mínimos locales, lo que indica que los resultados óptimos pueden estar fuertemente influenciados por los valores iniciales de $x$ o las condiciones de inicio de los algoritmos. Por esta razón, para evaluar el rendimiento y el comportamiento de los algoritmos en un entorno más general, se realizarán múltiples ejecuciones. En cada corrida, los algoritmos partirán de valores iniciales distintos generados aleatoriamente. Para evaluar el rendimiento de los métodos de optimización desarrollados, se harán 20 corridas para cada función, y se tomará para cada uno el tiempo de ejecución y el mejor fitness encontrado hasta el momento.

#### Gráficos de mejor fitness para los diferentes métodos

Para cada función se realizó un gráfico de comparación de mejor fitness de cada método para cada corrida, dándo el siguiente resultado

![](images/rendimiento_funciones.png){fig-align="center"}

y en cuanto a los tiempos, de obtuvieron los siguientes resultados generales
...
-   **Gradiente Descendente**: Es consistentemente el método más rápido, obtuvo tiempos de ejecución de entre 0.003 a 0.008 segundos, es extremadamente rápido en comparación pero potencialmente menos preciso

-   **Enjambre de Partículas**: Tiene tiempos de ejecución moderados de 0.07 a 0.27 segundos, tiene una sobrecarga computacional ligeramente mayor que Gradiente Descendente y presenta buen equilibrio entre velocidad y exploración

-   **Evolución Diferencial**: Tiene un rendimiento más lento con tiempos de ejecución de entre 0.8 y 1.9 segundos, consume más tiempo significativamente, lo que sugiere operaciones de mutación y cruce más complejas

-   **Algoritmo Genético**: Posee un rendimiento similar a Enjembre de Partículas con tiempos de ejecución de entre: 0.07 y 0.14 segundos, es más eficiente que Evolución Diferencial, lo que sugiere requisitos computacionales moderados
...
-   Se observa también en las gráficas, sobretodo para los métodos heurísticos, que entre más corridas se hagan, mejores resultadospodría ir consiguiendo , esto debido al factor aleatorio presente en éstos métodos